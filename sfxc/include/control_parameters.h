...
public:
  ...
  public:
    ...
    // Due to dispersion the central frequency of each channel mis-alligned in time
  ...
  // Because of dispersion, consecutive time slices overlap. The overlapping section are buffered
  Time buffer_time;
...
public:
  ...
  // maps pulsar name to vector of pulsar parameters
...
public:
   ...
   * from the correlator node to the output_node.
   **/
  static int nr_ffts_to_output_node
  (const Time &integration_time,
   int sample_rate,
   int fft_size_correlation){ 
     Time time_fft(fft_size_correlation / (sample_rate / 1000000.));
     return floor(integration_time / time_fft);
  }

  // Number of samples to be transferred between input and correlator node
  static int nr_samples_per_slice
  (const Time &integration_time,
   int sample_rate,
   ...
   int fft_size_correlation){ 
     ...
     int nr_corr_fft = nr_ffts_to_output_node(integration_time, sample_rate, 
