...
void Delay_correction::do_task() {
  ...
  // there are fft_size+1 points and each fft should be 16 bytes alligned 
  int output_stride =  fft_dedisp_size()/2 + 4; 
  ...
  for (int i = 0; i < nfft_cor; i++) {
    ...
    memcpy(&cur_output->data[i * output_stride], 
           &temp_fft_buffer[output_offset], 
           output_stride * sizeof(std::complex<FLOAT>));
    }
  ...
  Delay_correction::set_parameters(const Correlation_parameters &parameters, 
    ...
    if (fft_dedisp_size() > fft_rot_size())
      temp_fft_buffer.resize(fft_dedisp_size()/2 + 4);
    else
    ...
    if (coherent_dedispersion) {
      ...
      if (delta > 0) {
        output_offset = delta * fft_dedisp_size() / parameters.sample_rate;
        temp_fft_offset = 0;
      } else {
        output_offset = 0;
        temp_fft_offset = -delta * fft_dedisp_size() / parameters.sample_rate;
      }
    }
