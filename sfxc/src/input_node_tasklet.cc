...
add_time_interval(Time &start_time, Time &stop_time) {
  ...
  Time delay_start = Time(akima_delays.delay(start_time)*1e6) - buffer_time;
  Time delay_stop = Time(akima_delays.delay(stop_time)*1e6) + max_channel_offset + buffer_time*3;
