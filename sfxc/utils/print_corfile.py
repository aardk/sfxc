...
def read_baselines(infile, data, nbaseline, nchan, fmt):
  if fmt == 2:
    # data is in phased array format
    baseline_data_size = (nchan + 1) * 4
    blfmt = str((nchan+1)) + 'f'
  if (fmt == 3) or (fmt == 4):
    # data is in BOLO / VOLTAGE format
  ...
  for b in range(nbaseline):
    ...
    J = complex(0,1)
    if fmt == 2:
      buf = struct.unpack(blfmt, baseline_buffer[index:index + baseline_data_size])
      vreal = array(buf)
      vmin = vreal.min()
      vmax = vreal.max()
      vstd = vreal.std()
...
def read_time_slice(infile, stats, uvw, data, nchan, fmt):
  ...
  while current_slice == timeslice_header[0]:
    ...
    read_baselines(infile, data, nbaseline, nchan, fmt)
...
while True:
  ...
  data = {}
  try:
    read_time_slice(infile, stats, uvw, data, nchan, fmt)
    nslices += 1
  except Exception, e:
    if e.args[0] != 'EOF':
      raise
